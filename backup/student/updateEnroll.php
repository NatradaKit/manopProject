...
if(isset($_FILES['slipImage']) && $_FILES['slipImage']['error'] === UPLOAD_ERR_OK) {
    $slipUUID = uniqid(); // สร้าง UUID สำหรับ slip
    $slipPath = '../admin/slip/' . $slipUUID . '_' . basename($_FILES['slipImage']['name']); // เก็บไฟล์ภาพลงใน directory ../../admin/slip
    // เก็บไฟล์ภาพลงใน directory ../../admin/slip
    move_uploaded_file($_FILES['slipImage']['tmp_name'], $slipPath);
} else {
...
$stmt->bindValue(':slip', $slipPath, SQLITE3_TEXT);
$result = $stmt->execute();
